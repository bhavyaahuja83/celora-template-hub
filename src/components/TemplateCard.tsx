...
const TemplateCard = ({ template }: TemplateCardProps) => {
  ...
  const { addToCart, saveToLibrary, isInCart, isInLibrary } = useCart();
  ...
  return (
    <Link to={`/template/${template.id}`}>
      <Card className="group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border-0 shadow-lg overflow-hidden bg-white animate-fade-in">
        <div className="relative overflow-hidden">
          ...
          >
            <Heart className={`w-4 h-4 ${isLiked || isInLibrary(template.id) ? 'fill-red-500 text-red-500' : 'text-gray-600'}`} />
          </button>
        </div>
        ...
        <CardFooter className="pt-0">
          <div className="flex space-x-2 w-full">
            ...
            <Button 
              ...
              size="sm"
            >
              {template.isFree ? 'Download' : isInCart(template.id) ? 'In Cart' : 'Add to Cart'}
            </Button>
          </div>
        </CardFooter>
      </Card>
